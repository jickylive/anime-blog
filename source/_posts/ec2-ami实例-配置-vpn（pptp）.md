...
## [EC2 AMI实例 配置 VPN（PPTP）](/?p=8001 "Permanent Link to EC2 AMI实例 配置 VPN（PPTP）") 五月 31st, 2011

由于朋友想要弄一个VPN，所以在EC2上的AMI实例上装了一个，下面是配置的步骤：

shell> sudo -i?

shell> yum -y install rpm-build gcc

shell> yum -y install ppp

shell> yum -y install make

shell> wget 'http://downloads.sourceforge.net/project/poptop/pptpd/pptpd-1.3.4/pptpd-1.3.4.tar.gz?r=http%3A%2F%2Fsourceforge.net%2Fprojects%2Fpoptop%2Ffiles%2Fpptpd%2F&ts=1306811287&use\_mirror=cdnetworks-kr-2'

shell>tar xzvf pptpd-1.3.4.tar.gz shell> cd pptpd-1.3.4
...
\# username server\_name “password” ip
